...
class HoverTooltip extends Component {
	...
	renderSVG(moreProps) {
		...
		const {  bgOpacity, backgroundShapeSVG, tooltipSVG } = this.props;
		...
		return (
			<g>
				<rect x={centerX - pointWidth / 2}
					...
					fill={"#FFFFFF"}
					opacity={bgOpacity} />
				<g className="react-stock-charts-tooltip-content" transform={`translate(${x}, ${y})`}>
